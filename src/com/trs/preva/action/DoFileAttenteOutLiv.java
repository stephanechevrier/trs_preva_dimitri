 ...
 *
 */
public class DoFileAttenteOutLiv 
{
	// 16/02/2021 - Crï¿½ation pour lot 3 PREVA : prises en charge
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutLiv.class.getName());
    ...
    private final String _tableRef = "LIGCOMM";

    
    public DoFileAttenteOutLiv(ProprieteCommune aPC)
    {
	...
	public int generateFile() throws TrsException
	{
		...
		sUpdateOrderSendLiv =    		"MERGE INTO a_proprietes aProp "
				+ "USING ( "
				+ "  SELECT "
	       		+ "    @NO_REF@ AS no_ref, "
	       		+ "    ''TRS_LIV_PREVA'' AS nom "
				+ "  FROM dual "
				+ "  ) virtuel on ( "
		...
		hLigcommAccess	myLCAccess = null;
		List<LigComm>	myLCList = null;
		
		SpotBatch			mySB = null;
		SpotToWebLiv		mySTW = null; // 18/02/2021
    	...
    	try { myLCAccess = new hLigcommAccess(sDbPropFileName, sDbName); }
    	catch ( TrsException e )
    	{
    	...
    	// **********************************************************************************
		myLog.info("LECTURE des Livraisons a prendre en compte");
		
        
        	try {
        		
				myLCList = myLCAccess.getSqlLiv(sDateCreation,this._tableRef, iNbJours);
        	...
        	{
    			myLog.info("  - " + myLCList.size() + " SAV trouve(s)");
    			
    			if ( myLCList.size() > 0 )
    			{
    				// Si des SAV, le fichier est ï¿½crit
    				
    				try { mySTW = new SpotToWebLiv(myLCList, this.myPC, sDbPropFileName, sDbName); }
    				catch ( Exception e )
    				{ 
    					myLog.fatal(e.toString());
    					return -2;
    			...
    			sDateExport = mySDF.format(myDate);
    	         ...
    	         for ( LigComm myLigComm : myLCList )
    	        {
    	        	myLog.debug("LIGCOMM #" + myLigComm.get_refCommande()+ " : MERGE a_proprietes TRS_LIV_PREVA correspondant");

    				// Script de mise ï¿½ jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendLiv.replaceAll("@NO_REF@", "" + myLigComm.get_refCommande());
