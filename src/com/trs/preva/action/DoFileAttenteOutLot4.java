 ...
 *
 */
public class DoFileAttenteOutLot4 
{
	// 28/12/2019 - Création pour lot 4 BL + Confirmation
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutLot4.class.getName());
    ...
    public DoFileAttenteOutLot4(ProprieteCommune aPC)
    {
    	this.myPC = aPC;
	...
	public int putFile() throws TrsException
	{
        ...
        String	sAnnee = "";
        String	sDirArchive = "";
        String	sDirArchiveBL = "";
        String	sDirFileAttente = "";
        String	sDirFileAttenteBL = "";
        String	sFileName = "";
        String 	sMois = "";
        String	sPropKey = "";
		...
		// Répertoire de stockage des fichiers à envoyer   preva_spotToWeb_bl_directory_propKey
		// ********************************************************************************************************************
		...
		sDirFileAttenteBL = this.myPC.getProperty(this.myPC.getEnvironnement() + "." + ProprieteCommune.preva_spotToWeb_bl_directory_propKey + ".target", "");
		 
		if ( sDirFileAttente.equals("") )
		{
			myLog.fatal(this.myPC.getMessageErreur(1, this.myPC.getEnvironnement() + "." + ProprieteCommune.preva_spotToWeb_bl_directory_propKey + ".target"));
			return -1;
		}
		...
		sDirArchiveBL = this.myPC.getProperty(ProprieteCommune.dir_out_archive_propKey, "");
		...
		myFileUtil.makeDir(sDirArchiveBL);
        ...
        String sFichiersBL[] = myFileUtil.getFichiers(sDirFileAttenteBL, "");
		...
		myLog.info("LECTURE dans [" + sDirFileAttenteBL + "]");
		
		
		if ( sFichiersBL.length > 0 ) 
		{
	        // Si environnement de production ou de recette, on récupère les informations dans le fichier de propriétés
			if ( this.myPC.getEnvironnement().equals("prod") || this.myPC.getEnvironnement().equals("rec") )
			{
			...
			try
			{
				...
				for ( int j = 0; j < sFichiersBL.length; j ++ ) 
				{
				    try 
					{
						myLog.info("");
						myLog.info("Fichier [" + sFichiersBL[j] + "]");
				        
				        myLog.info("  - avant PUT");
				
				        
				        sFileName = sDirFileAttenteBL + "/" + sFichiersBL[j];
				        mySFtpClient.uploadFile(sFileName, sFichiersBL[j], WriteMode.OVERWRITE);
				        ...
				        myFileUtil.makeDir(sDirArchiveBL + "/" + "blToWeb");
						myFileUtil.copyBytes(sFileName, sDirArchiveBL + "/" + "blToWeb" + "/" + sFichiersBL[j], true);
