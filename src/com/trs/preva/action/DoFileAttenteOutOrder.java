 ...
 *
 */
public class DoFileAttenteOutOrder 
{
	// 16/02/2021 - Crï¿½ation pour lot 3 PREVA : prises en charge
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutOrder.class.getName());
    ...
    public DoFileAttenteOutOrder(String iProp,ProprieteCommune aPC)
    {
	...
	public int generateFile() throws TrsException
	{
		...
		sUpdateOrderSendOrder=    		"MERGE INTO a_proprietes aProp "
				+ "USING ( "
				+ "  SELECT "
	       		+ "    @NO_REF@ AS no_ref, "
	       		+ "    ''TRS_ORDER_PREVA''  AS nom, "
				...
				// Numï¿½ro commande concernï¿½e
	       		+ "@NO_OT@, "
	       		+ "''@EVENEMENT@'', "
	       		+ " ''VALEUR1_DOUBLE = Numero OT | VALEUR2_CHAINE = EVENEMENT ''"
		...
		hLigcommAccess	myLGAccess = null;
		List<hLigcomm>	myLGList = null;
		
		SpotBatch			mySB = null;
		SpotToWebOrder		mySTW = null; // 18/02/2021
    	...
    	try { myLGAccess = new hLigcommAccess(sDbPropFileName, sDbName); }
    	catch ( TrsException e )
    	{
    	...
    	// **********************************************************************************
		myLog.info("LECTURE des en-tï¿½te de commande a prendre en compte");
		
        
        	try {
				myLGList = myLGAccess.getSqlForOrder(sDateCreation,iProp,this._tableRef, iNbJours);
        	...
        	{
    			myLog.info("  - " + myLGList.size() + " SAV trouve(s)");
    			
    			if ( myLGList.size() > 0 )
    			{
    				// Si des SAV, le fichier est ï¿½crit
    				
    				try { mySTW = new SpotToWebOrder(iProp,myLGList, this.myPC, sDbPropFileName, sDbName); }
    				catch ( Exception e )
    				{ 
    					myLog.fatal(e.toString());
    					return -2;
    	       ...
    	       for ( LigComm myLigComm : myLGList )
    	        {
    	        	myLog.debug("P_OP #" + myLigComm.get_refCommande()  + " : MERGE a_proprietes TRS_ORDER_PREVA correspondant");

    				// Script de mise ï¿½ jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendOrder.replaceAll("@NO_REF@", "" + myLigComm.get_refCommande());
    				
    				sTemp = sTemp.replaceAll("@NO_OT@", "" + myLigComm.get_ot().getNoOt());
    				// Date de mise ï¿½ jour
