 ...
 *
 */
public class DoFileAttenteOutMlv 
{
	// 16/02/2021 - Crï¿½ation pour lot 3 PREVA : prises en charge
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutMlv.class.getName());
    ...
    public DoFileAttenteOutMlv(String iProp,ProprieteCommune aPC)
    {
	...
	public int generateFile() throws TrsException
	{
		...
		sUpdateOrderSendMlv =    		"MERGE INTO a_proprietes aProp "
				+ "USING ( "
				+ "  SELECT "
	       		+ "    @NO_REF@ AS no_ref, "
	       		+ "    ''TRS_MLV_PREVA''  AS nom, "
	       		+ "    @NO_OT@ as VALEUR1_DOUBLE,"
	       		+ " 	@NOID@ as VALEUR2_DOUBLE"
				+ "  FROM dual "
				+ "  ) virtuel on ( "
				+ "   aProp.no_ref = virtuel.no_ref "
				+ "  AND aProp.nom = virtuel.nom "
				+ "  AND aProp.VALEUR1_DOUBLE = virtuel.VALEUR1_DOUBLE "
				+ "  AND aProp.VALEUR2_DOUBLE = virtuel.VALEUR2_DOUBLE "
				+ "  ) "
				...
				// Numï¿½ro commande concernï¿½e
	       		...
	       		+ "@NOID@, "
	       		+ "''@TYPE_ENREGISTREMENT@'', "
	       		+ " ''VALEUR1_DOUBLE = Numï¿½ro OT |VALEU2_DOUBLE = Numï¿½roLigne | VALEUR2_CHAINE = TYPE ENREGISTREMENT ''"
				+ ") "
				;
		...
		SpotBatch			mySB = null;
		SpotToWebMlv		mySTW = null; // 18/02/2021
    	...
    	// **********************************************************************************
		myLog.info("LECTURE des MLV a prendre en compte");
		
        
        	try {
        		myLog.info("argument : " + iProp);
				myP_OPList = myP_OPAccess.getSqlforMLV(sDateCreation,iProp,this._tableRef, iNbJours);
        	...
        	else
        	{
    			...
    			if ( myP_OPList.size() > 0 )
    			{
    				...
    				try { mySTW = new SpotToWebMlv(iProp,myP_OPList, this.myPC, sDbPropFileName, sDbName); }
    				catch ( Exception e )
    				{ 
    					myLog.fatal(e.toString());
    					return -2;
    	       ...
    	       for ( hP_OP myP_OP : myP_OPList )
    	        {
    	        	myLog.debug("P_OP #" + myP_OP.get_refCommande() + " : MERGE a_proprietes TRS_MLV_PREVA correspondant");

    				// Script de mise ï¿½ jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendMlv.replaceAll("@NO_REF@", "" + myP_OP.get_refCommande());
    				sTemp = sTemp.replaceAll("@NO_OT@", "" + myP_OP.get_OT().getNoOt());
    				sTemp = sTemp.replaceAll("@NOID@", "" + myP_OP.get_OT().get_hHistoSav().get_noID());
    				// Date de mise ï¿½ jour
    				sTemp = sTemp.replaceAll("@VALEUR1_DATE@", "sysdate");
    				sTemp = sTemp.replaceAll("@TYPE_ENREGISTREMENT@","" + myP_OP.get_enregistrement() );
