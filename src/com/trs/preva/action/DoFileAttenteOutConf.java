 ...
 *
 */
public class DoFileAttenteOutConf 
{
	// 16/02/2021 - Création pour lot 3 PREVA : prises en charge
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutConf.class.getName());
    ...
    public DoFileAttenteOutConf(String iProp,ProprieteCommune aPC)
    {
	...
	public int generateFile() throws TrsException
	{
        ...
        String 	sDirConf = "";
        String	sGrosVolume = "";
        String	sUpdateOrderSendConfExplicite = "";
        String	sUpdateOrderSendConfImplicite = "";
        String	sPropKey = "";
		String	sRetour = "";
		...
		sUpdateOrderSendConfExplicite =    		"  UPDATE HISTOSAV "
				+ "                       SET CHAMPS1 = to_char(@VALEUR1_DATE@)  "
				+ "                       WHERE NO_OT = @NO_OT@  "
				+ "                       AND CODE_SITUATION = ''RST''  "
				+ "                       AND CODE_JUSTIFICATION = ''LDF'' "
			;
		
		sUpdateOrderSendConfImplicite =    		"  UPDATE HISTOSAV "
				+ "                       SET CHAMPS1 = to_char(@VALEUR1_DATE@)  "
				+ "                       WHERE NO_OT = @NO_OT@  "
				+ "                       AND CODE_SITUATION = ''MLV''  "
				+ "                       AND CODE_JUSTIFICATION = ''CFM'' "
			;
		...
		SpotBatch			mySB = null;
		SpotToWebConf		mySTW = null; // 18/02/2021
    	...
    	// **********************************************************************************
		myLog.info("LECTURE des Confirmations de RDV a prendre en compte");
		
        
        switch(iProp){
        	
        	case "ConfirmationExplicite" : try {
				myHSList = myHSAccess.getHistoSavForConf("ConfirmationExplicite",this._tableRef, iNbJours);
        	...
        	else
        	{
    			...
    			if ( myHSList.size() > 0 )
    			{
    				...
    				try { mySTW = new SpotToWebConf(myHSList, this.myPC, sDbPropFileName, sDbName); }
    				catch ( Exception e )
    				{ 
    					myLog.fatal(e.toString());
    					return -2;
    	        ...
    	        {
    	        	myLog.debug("HISTOSAV #" + mySav.get_noID() + " : MERGE a_proprietes TRS_BL_PREVA correspondant");

    				// Script de mise à jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendConfExplicite.replaceAll("@NO_OT@", "" + mySav.get_noOT());
        	...
        	case "ConfirmationImplicite": try {
				myHSList = myHSAccess.getHistoSavForConf("ConfirmationImplicite",this._tableRef, iNbJours);
			} catch (AppliException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			} 
        	...
        	else
        	{
    	        ...
    	        for ( HistoSav mySav : myHSList )
    	        {
    	        	myLog.debug("HISTOSAV #" + mySav.get_noID() + " : MERGE a_proprietes TRS_CONF_PREVA correspondant");

    				// Script de mise à jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendConfImplicite.replaceAll("@NO_OT@", "" + mySav.get_noOT());
