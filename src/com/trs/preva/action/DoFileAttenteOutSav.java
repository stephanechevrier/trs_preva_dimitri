 ...
 *
 */
public class DoFileAttenteOutSav 
{
	// 16/02/2021 - Cr�ation pour lot 3 PREVA : prises en charge
	
	// Log
	static final Logger myLog = LogManager.getLogger(DoFileAttenteOutSav.class.getName());
    ...
    public DoFileAttenteOutSav(ProprieteCommune aPC)
    {
     ...
     * 
     * @return -1 si erreur, -2 si erreur � la génération du fichier, SINON le nombre de fichiers traités
     */
	public int generateFile() throws TrsException
	{
		...
		sUpdateOrderSendSav =    		"MERGE INTO a_proprietes aProp "
				+ "USING ( "
				+ "  SELECT "
	       		+ "    @NO_REF@ AS no_ref, "
	       		+ "    ''TRS_SAV_PREVA''  AS nom, "
				...
				// Num�ro commande concern�e
	       		+ "@NO_OT@, "
	       		+ "''@EVENEMENT@'', "
	       		+ " ''VALEUR1_DOUBLE = Num�ro OT | VALEUR2_CHAINE = EVENEMENT''"
		...
		hHistosavAccess	myHSAccess = null;
		List<hHistoSav>	myHSList = null;
		
		SpotBatch			mySB = null;
		SpotToWebSav		mySTW = null; // 18/02/2021
		String 				no_ot = "";
		...
		// Date de saisie des commandes � partir duquelle on prend en compte les donn�es � envoyer
		// ********************************************************************************************************************
    	...
    	try { myHSAccess = new hHistosavAccess(sDbPropFileName, sDbName); }
    	catch ( TrsException e )
    	{
		...
		myLog.info("LECTURE des SAV a prendre en compte");
        	...
        	try {
				myHSList = myHSAccess.getSqlforSav(sDateCreation,this._tableRef, iNbJours);
        	...
        	else
        	{
    			...
    			if ( myHSList.size() > 0 )
    			{
    				// Si des SAV, le fichier est �crit
    				
    				try { mySTW = new SpotToWebSav(myHSList, this.myPC, sDbPropFileName, sDbName); }
    				catch ( Exception e )
    				{ 
    					myLog.fatal(e.toString());
    					return -2;
    	        ...
    	        for ( HistoSav myHS : myHSList )
    	        {
    	        	myLog.debug("HISTOSAV #" + myHS.getLigComm().get_refCommande() + " : MERGE a_proprietes TRS_SAV_PREVA correspondant");

    				// Script de mise � jour SQL pour SpotBatch : table A_PROPRIETES
    				// *************************************************************************************
    				sTemp = sUpdateOrderSendSav.replaceAll("@NO_REF@", "" + myHS.getLigComm().get_refCommande());
    				sTemp = sTemp.replaceAll("@NO_OT@", no_ot.valueOf(myHS.get_noOT()));

    				// Date de mise � jour
    				sTemp = sTemp.replaceAll("@VALEUR1_DATE@", "sysdate");
    				sTemp = sTemp.replaceAll("@EVENEMENT@", "" + myHS.get_codeSituation() + "-" + myHS.get_codeJustification());
